...
class PayrollEntry(Document):
	...
	def get_salary_components(self, component_type, salary_slips):
		if not salary_slips:
	...
	def get_salary_component_total(self, component_type = None, salary_slip = None):
	...
	def make_accrual_jv_entry(self):
		...
		for salary_slip in salary_slips:
			employee = frappe.db.sql(""" SELECT * FROM `tabEmployee` WHERE name=%s""", salary_slip.employee, as_dict=True)
			earnings = self.get_salary_component_total(component_type = "earnings",salary_slip = salary_slip) or {}
			...
			if earnings or deductions:
				...
				for acc, amount in earnings.items():
					...
					accounts.append({
							...
							"party_type": '',
							"cost_center": employee[0].cost_center if employee[0].cost_center else self.cost_center ,
